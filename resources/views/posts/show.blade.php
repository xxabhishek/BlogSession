                        ...
                        <div class="border-bottom pb-2 mb-2">
                            ...
                            @auth
                                <form method="POST" action="{{ route('comments.like', $comment->id) }}"
                                    ...
                                    @csrf
                                    <button type="submit" class="btn btn-info btn-sm">Like
                                        ({{ $comment->likes->count() }})</button>
                                </form>
                                <form method="POST" action="{{ route('comments.share', $comment->id) }}"
                                    style="display:inline;">
                                    @csrf
                                    <button type="submit" class="btn btn-success btn-sm">Share
                                        ({{ $comment->shares->count() }})</button>
                                </form>
                                @can('delete', $comment)
            ...
            $('#comment-form').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#comment-list').append(
                            '<li><strong>' + response.user.name + ':</strong> ' + response
                            .comment.content + '</li>'
                        );
